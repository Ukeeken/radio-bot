...
# FIX #6: Use lock everywhere song_requests is mutated (Flask + Discord + loop)
lock = threading.Lock()
...
def status():
    ...
    return jsonify({
        ...
        "artist": artist,
        "title": title,
        # FIX #3: was recent_songs[:-1] which drops the newest entry — use full list
        "requests": song_requests[-3:],
        "recent": recent_songs,
        "album_art": last_album_art
    })
...
def request_song():
    ...
    try:
        ...
        # FIX #6: lock used here (was already correct in Flask route — kept)
        with lock:
...
@app.route("/player")
def player():
    # FIX #5: wrap file reads in try/except so missing files return a clear error
    try:
...
@app.route("/schedule")
def schedule():
    # FIX #5: same fix for schedule page
    try:
        return open("dj-schedule.html").read()
    except FileNotFoundError:
        return "Schedule page not found.", 404
...
class DJPanel(discord.ui.View):
    ...
    async def start_dj(
        ...
        try:
            ...
            artist, title = await get_now_playing_async()
            if title != "Unknown":
    ...
    async def clear_requests_button(
        ...
        # FIX #6: lock mutation of song_requests
        with lock:
            song_requests.clear()
            requests_updated = True
            force_refresh = True
...
# FIX #7: async wrapper so get_now_playing never blocks the event loop
async def get_now_playing_async():
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, get_now_playing)


# FIX #2: removed dead spotify_enrich function — it fetched data but returned
# the original values unchanged. Remove this comment too once you're happy.
...
def create_embed(artist, title, dj, album_art):
    ...
    if song_requests:
        ...
        for r in latest_requests:
            req_song = r.get("song", "Unknown Song")
            # FIX #1: renamed loop var from `artist` to `req_artist` to avoid
            # shadowing the `artist` function parameter used in the embed above
            req_artist = r.get("artist", "Unknown Artist")
            req_user = r.get("user", "Unknown")
            req_server = r.get("server", "Unknown")

            request_lines.append(
                f"• 🎵 **{req_artist} - {req_song}**\n"
                f"  👤 {req_user}\n"
                f"  🌐 {req_server}"
            )
...
async def delete_old_message(guild_id):
    ...
    channel_id = radio_channels.get(guild_id)
...
async def post_scroller(artist, title):
    ...
    for guild in client.guilds:
        ...
        # FIX #4: use str key consistently
        ...
        try:
            ...
            # FIX #4: store with str key
            last_messages[str(guild.id)] = msg.id

            print(f"Posted in {guild.name}")

            # Minor: small delay between guilds to avoid Discord rate limits
...
async def test_voice(interaction: discord.Interaction):
    ...
    # FIX: added owner-only guard — previously any user could invoke this
    if interaction.user.id != OWNER_ID:
        await interaction.response.send_message(
            "❌ Owner only.",
            ephemeral=True
...
async def dj_end(interaction: discord.Interaction):
    ...
    # FIX: removed the redundant second clear_all_scrollers() + sleep(2) —
    # that was a workaround for the blocking event loop issue, now resolved.
...
async def request_song_command(
    ...
    # FIX #6: lock mutation from Discord slash command — was missing before
    with lock:
        song_requests.append({
            "song": song,
            "artist": artist,
    ...
    while not client.is_closed():
        ...
        try:
            ...
            if current_dj:
                ...
                # FIX #7: use async wrapper — was blocking the event loop
                artist, title = await get_now_playing_async()
                ...
                if should_update:
                    # FIX #6: reset flags inside lock before posting so a
                    # concurrent Flask/Discord write isn't lost
                    with lock:
                        requests_updated = False
                        force_refresh = False
                    await post_scroller(artist, title)
                ...
                # FIX #4: last_messages uses str keys — check len() not truthiness
